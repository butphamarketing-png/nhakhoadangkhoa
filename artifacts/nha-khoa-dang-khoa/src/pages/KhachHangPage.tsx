          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {TESTIMONIALS.map((t, i) => (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3 pt-3 border-t border-black/[0.06]">
                  <div>
                    <div className="font-bold text-[#0D1B2A] text-sm">{t.name}</div>
                    <div className="text-[#C89B3C] text-xs font-semibold mt-0.5">{t.service}</div>
                  </div>
                  <div className="text-xs text-gray-400 text-right shrink-0">{t.date}</div>
                </div>
              </motion.div>
            ))}
          </div>
